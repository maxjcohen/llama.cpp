...
enum mmvq_parameter_table_id {
    ...
    MMVQ_PARAMETERS_PASCAL,   // sm_60–sm_69: fewer warps/block improves occupancy on low-SM-count GPUs
    MMVQ_PARAMETERS_GCN,
...
#elif defined(__CUDA_ARCH__) && __CUDA_ARCH__ >= 600 && __CUDA_ARCH__ < 700
...
static __host__ mmvq_parameter_table_id get_device_table_id(int cc) {
    ...
    if (GGML_CUDA_CC_IS_NVIDIA(cc) && cc >= GGML_CUDA_CC_PASCAL && cc < GGML_CUDA_CC_VOLTA) {
        return MMVQ_PARAMETERS_PASCAL;
    }
    return MMVQ_PARAMETERS_GENERIC;
}
